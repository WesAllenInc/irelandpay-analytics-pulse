  ...
  const fetchAnalytics = async (): Promise<void> => {
    try {
      ...
      if (syncData) {
        ...
        const durations = completedSyncs.map(s => {
        ...
        const merchantCounts = completedSyncs
          .filter(s => s.results?.merchants_count)
          .map(s => s.results.merchants_count || 0)
        
        const residualCounts = completedSyncs
          .filter(s => s.results?.residuals_count)
          .map(s => s.results.residuals_count || 0)
        
        const totalMerchants = merchantCounts.reduce((sum, count) => sum + count, 0)
        const totalResiduals = residualCounts.reduce((sum, count) => sum + count, 0)
